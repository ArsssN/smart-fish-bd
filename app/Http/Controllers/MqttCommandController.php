...
class MqttCommandController extends Controller
{
     ...
     * @return string
     */
    public static function getMqttData(string $type, object $responseMessage): string
    {
        ...
        $typeUnit->{$type . 'Types'}
            ->each(
                function ($typeType)
                use ($type, $responseMessage, &$switchState) {
                    self::getMqttDataHistory($typeType, $type, $responseMessage, $switchState);
                }
            );
     ...
     * @return void
     */
    private static function getMqttDataHistory($typeType, $type, $responseMessage, &$switchState): void
    {
    ...
    private static function saveMqttDataHistory($typeType, $typeUnit, $type, $responseMessage, $mqttData, &$switchState): void
    {
        ...
        $typeUnit->ponds->each(
            function ($pond)
            use ($type, $mqttData, $typeUnitID, $typeTypeID, $value, $type_message) {
                $pondID = $pond->id;
    ...
    private static function changeSwitchStateOfSensorUnit($mqttData, $ponds, $switchState): void
    {
        $ponds->each(function ($pond) use ($mqttData, $switchState) {
            ...
            $pond->switchUnits->each(
                function ($switchUnit)
                use ($mqttData, $switchState, &$mqttDataSwitchUnitHistories) {

                    $switches = collect($switchUnit->switches ?? '[]');
                    $switches->map(function ($switch, $index) use ($switchState) {
