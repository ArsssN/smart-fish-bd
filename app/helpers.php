...
// get Sensor Types average based on time
if (!function_exists('getSensorTypesDailyAverageBasedOnTime')) {
    /**
     * @param $historiesQuery
     * @param $lastDate
     * @param $laterDate
     * @return object
     */
    function getSensorTypesDailyAverageBasedOnTime($historiesQuery, $lastDate = null, $laterDate = null): object
    {
        $lastDate = $lastDate ?? now()->subDay();
        $laterDate = $laterDate ?? now();
        return $historiesQuery->where('sensor_unit_id', '!=', null)->latest()
